...
def word2vec(test2):
   ...
   wordsnithya = list(word2vec.wv.key_to_index)
   sentences = [row.split(',') for row in col_titlesentences]
   model = Word2Vec(sentences, min_count=1,vector_size= 50,workers=3, window =3, sg = 1)
   model.wv['A novel Injection Locked Rotary Traveling Wave Oscillator']
   word2vecInput='A novel Injection Locked Rotary Traveling Wave Oscillator'
   word2vecOutput=model.wv.most_similar(word2vecInput)[:5]
   return word2vecOutput

def build_model_knn(test2):
    neigh = NearestNeighbors ( n_neighbors=5 )
    global featurevectors
    # Convert featurevectors to a numpy array
...
def recommend_collaborative_filtering(list_of_papers):
    ...
    target_references = []
    count = 0
    with open('papers.csv', encoding='ISO-8859-1', errors='replace') as csvfile:
        readCSV = csv.DictReader(csvfile, delimiter=',')

        for row in readCSV:
            if count > 0:
                if row['id'] in list_of_papers:
    ...
    with open('papers.csv', encoding='ISO-8859-1', errors='replace') as csvfile:
        ...
        for row in readCSV:
            if count > 0:
                ...
                id_title_dict[row['id']] = row['title']

                refs = row['references'].split(',')
                for r in refs:
                    if r in list_of_papers:
                        candidate_papers.append(row['id'])
